<?php
	include "koneksi.php";
	$kode= $_GET['kode'];
	$queryEdit = mysqli_query($connection,"SELECT * FROM user where iduser='$kode' limit 1")or die(mysqli_error());
	$dataEdit = mysqli_fetch_array($queryEdit);
?>
<div id="konten">
<h1>Data user</h1>  
<form method="POST" action="media_admin.php?module=update_proses_user&amp;kode=<?php echo $dataEdit['iduser'];?>" align="center" onsubmit="return validasi(this)">
	<table cellpadding="0" cellspacing="0" border="0">
		<tr>
			<td width="20%">iduser</td>
			<td>:</td>
			<td>
				<input type="text" name="inputiduser" size="40%" value=<?php echo $dataEdit['iduser'];?>>
			</td>
		</tr>
		<tr>
			<td width="20%">namauser</td>
			<td>:</td>
			<td><input type="text" name="inputnamauser" size="40%" value=<?php echo $dataEdit['namauser'];?>></td>
		</tr>
		<tr>
			<td width="20%">password</td>
			<td>:</td>
			<td><input type="text" name="inputpassword" size="40%" value=<?php echo $dataEdit['password'];?>></td>
		</tr>
		<tr>
			<td width="20%">hakakses</td>
			<td>:</td>
			<td><input type="text" name="inputhakakses" size="40%" value=<?php echo $dataEdit['hakakses'];?>></td>
		</tr>
	
		<tr>
			<td></td>
			<td></td>
			<td><input type="submit" name="tambahuser" value="Update Pelanggan">
